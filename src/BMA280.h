...
//CHANGE NAME TO BE COMPATIBLE WITH SENTRAL LIB
#define AAFS_2G 0x02
#define AAFS_4G 0x05
#define AAFS_8G 0x08
#define AAFS_16G 0x0C
...
public:
  ...
  // void MotionDetection(float threshold_x,float threshold_y, float threshold_z, int interval);
  void Calibrate();
  double GetAverage(float reading[]);
  ...
  byte readByte(uint8_t address, uint8_t subAddress);
  void readBytes(uint8_t address, uint8_t subAddress, uint8_t acount, uint8_t *dest);
  void setinterrupt();

private:
  uint8_t _intPin1;
  uint8_t _intPin2;
  double _center_x, _center_y, _center_z;
  float _aRes;
};
